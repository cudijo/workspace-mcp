...
A Google **Workspace** MCP server for Claude: Gmail, Calendar, Chat, Drive, Meet and the
Workspace directory. Works with **any Google account** — no gcloud CLI, no npm install, no
external dependencies. Just Node.js (>= 18) and a Google Cloud OAuth client.

> **About this fork.** This extends [grahac/google-mcp](https://github.com/grahac/google-mcp)
> (Gmail + Calendar, 5 tools) to 20 tools, adding Google Chat, Drive (search, change feed,
> document export, comments, activity, upload/update), Meet conference records and transcripts,
> and Workspace directory lookup. The additions follow the upstream design exactly: hand-rolled
> HTTPS, zero dependencies, no SDK.
>
> Internal identifiers are deliberately unchanged — the package, plugin name, `GOOGLE_MCP_CONFIG`
> and `~/.config/google-mcp/tokens.json` all keep their upstream names, so this stays a drop-in
> replacement and the additions remain cleanly upstreamable.
